            ...
            <div class="pt-8 border-t border-slate-200 dark:border-slate-800">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                    ...
                    <a href="{{ route('report.show', $hist->uuid) }}" class="block group">
                        <div class="h-full flex flex-col gap-3 p-4 rounded-xl border {{ $hist->uuid === $report->uuid ? 'bg-primary/5 border-primary/20' : 'bg-white dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50' }} transition-all shadow-sm">
                            <div class="flex items-center justify-between gap-2">
                                <div class="text-xs font-bold {{ $hist->uuid === $report->uuid ? 'text-primary' : 'text-slate-700 dark:text-slate-300' }} font-mono">
                                    {{ substr($hist->commit_hash, 0, 7) }}
                                </div>
                                <div class="size-2 rounded-full {{ $hist->uuid === $report->uuid ? 'bg-primary' : 'bg-slate-300 dark:bg-slate-700' }}"></div>
                            </div>
